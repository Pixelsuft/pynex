...
class NHorizontalSlider:
    ...
    def draw(self, surface: pygame.Surface, delta: float, scroll_x: int, scroll_y: int) -> None:
        ...
        pygame.draw.rect(
            ...
            (self.x + scroll_x, self.y + self.bar_top + scroll_y, self.w, self.bar_height),
            1
        )
        pygame.draw.rect(
            surface,
            self.bar_color,
            (self.x + scroll_x + 1, self.y + self.bar_top + scroll_y + 1, self.w - 2, self.bar_height - 2)
        )
        pygame.draw.rect(
            surface,
            self.current_color,
            (self.x + scroll_x + (self.value / (self.max_value - self.min_value) * (self.w - self.slider_width)),
             self.y + scroll_y, self.slider_width, self.h)
        )
    ...
    def _on_mouse_move(self, event: pygame.event.Event, bind: bool) -> None:
        if self.bar_focused:
            ...
            value = x / (self.w - self.slider_width) * (self.max_value - self.min_value)
            ...
            if is_colliding_rect(
                (self.x + self.lsx + (self.value / (self.max_value - self.min_value) * (self.w - self.slider_width)),
                 self.y + self.lsy, self.slider_width, self.h),
                event.pos
            ):
    ...
    def _on_mouse_down(self, event: pygame.event.Event, bind: bool) -> None:
        if event.button == pygame.BUTTON_LEFT:
            if self.bar_hovered:
                ...
                self.last_x_focus = event.pos[0] - self.x - self.lsx - (self.value / (self.max_value - self.min_value) *
                                             (self.w - self.slider_width))
            else:
                self.down_it = True
                if event.pos[0] > (
                        self.x + self.lsx + (self.value / (self.max_value - self.min_value) *
