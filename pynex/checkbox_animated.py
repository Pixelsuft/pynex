...
class NAnimatedCheckBox:
    def __init__(
        ...
        self.current_color3 = NSimpleColorFade(None, self.color3, self.color3, self.animation_time, False)
    ...
    def draw(self, surface: pygame.Surface, delta: float, scroll_x: int, scroll_y: int) -> None:
        ...
        self.current_color3.draw(surface, delta, scroll_x, scroll_y)
        pygame.draw.rect(
            surface,
    ...
    def _on_mouse_leave(self, event: pygame.event.Event, bind: bool) -> None:
        ...
        self.current_color3.create(self.current_color3.color, self.color3, self.animation_time)
        if bind:
            self.on_mouse_leave(event.pos)  # type: ignore

    def _on_mouse_enter(self, event: pygame.event.Event, bind: bool) -> None:
        self.current_color1.create(self.current_color1.color, self.color1_hover, self.animation_time)
        self.current_color2.create(self.current_color2.color, self.color2_hover, self.animation_time)
        self.current_color3.create(self.current_color3.color, self.color3_hover, self.animation_time)
        if bind:
    ...
    def _on_mouse_down(self, event: pygame.event.Event, bind: bool) -> None:
        if event.button == pygame.BUTTON_LEFT:
            self.current_color1.create(self.current_color1.color, self.color1_focus, self.animation_time)
            self.current_color2.create(self.current_color2.color, self.color2_focus, self.animation_time)
            self.current_color3.create(self.current_color3.color, self.color3_focus, self.animation_time)
        if bind:
